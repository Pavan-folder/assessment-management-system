...
import { getAssessmentConfig, extractValueFromPath, getClassificationForValue } from '../../../config/assessmentConfig';
import { assessmentData, getAssessmentBySessionId } from '../../../data/data';
...
export async function generateReport(sessionId: string): Promise<ReportResult> {
  try {
    ...
    const data = getAssessmentBySessionId(sessionId);
...
function processAssessmentData(data: any, config: any): ProcessedReport {
  ...
  for (const sectionConfig of config.sections) {
    ...
    for (const fieldConfig of sectionConfig.fields) {
      const value = extractValueFromPath(data, fieldConfig.dataPath);

      let classification = undefined;
      if (fieldConfig.classification && value !== null && value !== undefined) {
        classification = getClassificationForValue(fieldConfig.fieldName, value);
      }
...
async function generateHTMLReport(processedReport: ProcessedReport, config: any): Promise<string> {
  const templatePath = path.join(__dirname, '../../templates', config.templateFile);
...
function populateTemplate(template: string, data: ProcessedReport): string {
  let html = template;
...
function generateDefaultTemplate(data: ProcessedReport): string {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
async function generatePDF(htmlContent: string, sessionId: string): Promise<string> {
  let browser;
  try {
